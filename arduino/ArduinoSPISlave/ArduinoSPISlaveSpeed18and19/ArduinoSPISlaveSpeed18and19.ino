...
//* PACK 28 bytes 224 bits        *
//* en 32 bytes                   *
//* 32 bytes send 28 bytes        *
//* Vel 18: 50 ms 4480 baud (*)   * 
//* Vel 19: 25 ms 8960 baud (#)   *
//* Espera 1050 ms vel 18         *
//* Espera 525 ms vel 19          *
//* Realterm delay 1050           *
//* Vel 18 - 25,20,18,17          *
//* Vel 19 - 25,20,18             *
//*********************************
...
void Speed19ms17NoFlag(void);
void Speed19ms18NoFlag(void);
void Speed19ms20NoFlag(void);
void Speed19ms25NoFlag(void);
void Speed18ms17Flag(void);
void Speed18ms18Flag(void);
void Speed18ms20Flag(void);
void Speed18ms25Flag(void);
...
//***************************************************************
void Speed18ms17Flag() //Envio pack 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
...
//***************************************************************
void Speed18ms18Flag() //Envio 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
{for (uint16_t i=0;i<gb_cont_buf;i+=32)
 {memcpy(&data_buff[2],&gb_buf[i],32);
  data_buff[1] = 0x01; //Activo
  _delay_us(18000);
...
//***************************************************************
void Speed18ms20Flag() //Envio 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
{for (uint16_t i=0;i<gb_cont_buf;i+=32)
 {memcpy(&data_buff[2],&gb_buf[i],32);
  data_buff[1] = 0x01; //Activo
  _delay_us(20000);
...
//***************************************************************
void Speed18ms25Flag() //Envio 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
{for (uint16_t i=0;i<gb_cont_buf;i+=32)
 {memcpy(&data_buff[2],&gb_buf[i],32);
  data_buff[1] = 0x01; //Activo
  _delay_us(25000);
...
//***************************************************************
void Speed19ms17NoFlag() //Envio 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
{for (uint16_t i=0;i<gb_cont_buf;i+=32)
 {memcpy(&data_buff[2],&gb_buf[i],32);
...
//***************************************************************
void Speed19ms18NoFlag() //Envio 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
{for (uint16_t i=0;i<gb_cont_buf;i+=32)
 {memcpy(&data_buff[2],&gb_buf[i],32);
  data_buff[1] = gb_flipflop; //Activo
  _delay_us(18000);
...
//***************************************************************
void Speed19ms20NoFlag() //Envio 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
{for (uint16_t i=0;i<gb_cont_buf;i+=32)
 {memcpy(&data_buff[2],&gb_buf[i],32);
  data_buff[1] = gb_flipflop; //Activo
  _delay_us(20000);
...
//***************************************************************
void Speed19ms25NoFlag() //Envio 28 bytes 32 bytes(quitamos parte alta fallo bit 7)
{for (uint16_t i=0;i<gb_cont_buf;i+=32)
 {memcpy(&data_buff[2],&gb_buf[i],32);
  data_buff[1] = gb_flipflop; //Activo
  _delay_us(25000);
...
void EnviaDatos()
{
 ...
 {//Con flag
  ...
  {
   default: break;
   case 17: Speed18ms17Flag(); break;
   case 18: Speed18ms18Flag(); break;
   case 20: Speed18ms20Flag(); break;
   case 25: Speed18ms25Flag(); break;
  }
 }
 else
 {//Sin flag
  ...
  {
   default: break;   
   case 17: Speed19ms17NoFlag(); break;
   case 18: Speed19ms18NoFlag(); break;
   case 20: Speed19ms20NoFlag(); break;
   case 25: Speed19ms25NoFlag(); break;   
  }    
 }
 gb_cont_buf=0;    
}
